...
keypoints = np.load('data/data_2d_' + args.dataset + '_' + args.keypoints + '.npz', allow_pickle=True) # data_2d_h36m_cpn_ft_h36m_dbb.npz
...
model_pos_train = Pose3D(args)
model_pos = Pose3D(args)

dummy_input = torch.randn(1, 27, 17, 2)
flops, params = profile(model_pos, (dummy_input,))
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            inputs_3d[:, :, 0] = 0
            optimizer.zero_grad()
            predicted_3d_pos, x2_loss, x3_loss = model_pos_train(inputs_2d)
            del inputs_2d
            torch.cuda.empty_cache()
            loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d) + (11/17) * mpjpe(x2_loss, inputs_3d) + (6/17) * mpjpe(x3_loss, inputs_3d)
            epoch_loss_3d_train += inputs_3d.shape[0] * inputs_3d.shape[1] * loss_3d_pos.item()
            # print('batch_loss_3d_train', inputs_3d.shape[0] * inputs_3d.shape[1] * loss_3d_pos.item())
        ...
        if args.no_eval:
            ...
            print('[%d] time' % (epoch + 1), "loss: ", round(losses_3d_train[-1] * 1000, 5))
            print('MPJPE：' + str(losses_3d_valid[-1] * 1000))
        lr *= lr_decay
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    print('Protocol #1 Error (MPJPE):', e1, 'mm', file=file)
    print('Protocol #2 Error (P-MPJPE):', e2, 'mm', file=file)
    print('Protocol #3 Error (N-MPJPE):', e3, 'mm', file=file)
    print('Velocity Error (MPJVE):', ev, 'mm', file=file)
    print('----------', file=file)
    file.flush()
    file.close()
    return e1, e2, e3, ev
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        file = open('output.txt', 'a')
        print('Protocol #1   (MPJPE) action-wise average:', round(np.mean(errors_p1), 1), 'mm', file=file)
        print('Protocol #2 (P-MPJPE) action-wise average:', round(np.mean(errors_p2), 1), 'mm', file=file)
        print('Protocol #3 (N-MPJPE) action-wise average:', round(np.mean(errors_p3), 1), 'mm', file=file)
        print('Velocity      (MPJVE) action-wise average:', round(np.mean(errors_vel), 2), 'mm', file=file)
        file.flush()
        file.close()
